...
class Batch_Loader(object):
    def __init__(self, train_triples, words_indexes, indexes_words, headTailSelector, \
        ...
        self.new_triples_indexes = np.empty((self.batch_size * (self.neg_ratio + 1), 3)).astype(np.int32) #128*2 (256, 3)
        self.new_triples_values = np.empty((self.batch_size * (self.neg_ratio + 1), 1)).astype(np.float32) # (256,1)
        #print("new_triples_indexes : " + str(self.new_triples_indexes)) #[[0 0 0] [0 0 0] .... .... [0 0 0]]

    #used here
    #def __call__(self, batch_num):
    def __call__(self):
        
        #print("Batch_Loader Batch_Loader Batch_Loader Batch_Loader Batch_Loader Batch_Loader")
        #print("batch_num in the function : " + str(batch_num))
        #batch_num = batch_num * 128
        idxs = np.random.randint(0, len(self.values), self.batch_size)
        #idxs = np.arange(batch_num, batch_num+self.batch_size)
        
        #print("idxs : " + str(idxs))
        self.new_triples_indexes[:self.batch_size, :] = self.indexes[idxs, :]
        #print("new_triples_indexes : " + str(self.new_triples_indexes[:self.batch_size, :]))
